...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EuropeCoinMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("EuropeCoinMiner: ProcessNewBlock, block not accepted");
...
{
	fGenerate = true;
    LogPrintf("EuropeCoinMiner started\n");
    srand(clock());
        ...
        while (true) {
            ...
            if(ma!=""){
            	if(validateAddress(ma)) {
           		    LogPrintf("EuropeCoinMiner: Mining to User supplied address %s\n",ma);
                	pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithAddress(ma));
                }else{
                	LogPrintf("EuropeCoinMiner: WARNING! User supplied address is invalid, defaulting to mempool address.\n");
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("EuropeCoinMiner: ERROR! Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running EuropeCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("EuropeCoinMiner terminated\n");
        delete [] scratchpad;
        fGenerate = false;
        return;
    }
    ...
    {
        LogPrintf("EuropeCoinMiner runtime error: %s\n", e.what());
        delete [] scratchpad;
        fGenerate = false;
        return;
    }
